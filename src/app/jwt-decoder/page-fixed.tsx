    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">JWT Decoder</h2>
              ...
              <div className="p-4">
                ...
                {activeTab === "about" && (
                  <div className="prose prose-indigo dark:prose-invert max-w-none">
                    ...
                    <h3>JWT Structure</h3>
                    <p>JWTs consist of three parts separated by dots (.): Header, Payload, and Signature.</p>
                    <pre className="bg-gray-100 dark:bg-gray-800 p-2 rounded text-sm">header.payload.signature</pre>
                    ...
                    <ul>
                      <li><strong>Registered claims</strong>: These are a set of predefined claims which are not mandatory but recommended, such as:
                        <ul>
                          <li><code>iss</code> (Issuer): Who issued the token</li>
                          <li><code>sub</code> (Subject): Who the token refers to</li>
                          <li><code>aud</code> (Audience): Recipients of the token</li>
                          <li><code>exp</code> (Expiration Time): When the token expires</li>
                          <li><code>nbf</code> (Not Before Time): When the token starts being valid</li>
                          <li><code>iat</code> (Issued At): When the token was issued</li>
                          <li><code>jti</code> (JWT ID): Unique identifier for the token</li>
                        </ul>
                      </li>
                      <li><strong>Public claims</strong>: These can be defined at will but to avoid collisions should be defined in the IANA JSON Web Token Registry or be defined as a URI.</li>
                      <li><strong>Private claims</strong>: These are custom claims created to share information between parties that agree on using them.</li>
                    </ul>
                    
                    <h4>3. Signature</h4>
                    <p>
                      To create the signature part, you need to take the encoded header, the encoded payload, a secret key, and the algorithm specified in the header, and sign that. The signature is used to verify that the message wasn't changed along the way, and, in the case of tokens signed with a private key, it can also verify that the sender of the JWT is who it says it is.
                    </p>
                    
                    <h3>How JWTs Work</h3>
                    <p>
                      The token is created on the server, usually when a user logs in or performs some action that requires authentication. The server signs the token with a secret key or a private key (in case of RSA). 
                    </p>
                    <p>
                      The client receives the token and stores it (typically in local storage, session storage, or cookies). For subsequent requests, the client includes the JWT, typically in the Authorization header using the Bearer schema.
                    </p>
                    <p>
                      The server then validates the JWT signature and processes the claims. If the token is valid, it grants access to the protected resources.
                    </p>
                    
                    <h3>Benefits of JWTs</h3>
                    <ul>
                      <li><strong>Compact</strong>: JWTs can be small in size, making them ideal for HTTP environments and URL parameters.</li>
                      <li><strong>Self-contained</strong>: The payload contains all the required information about the user, avoiding the need to query the database multiple times.</li>
                      <li><strong>Secure</strong>: JWTs can be signed, ensuring the integrity of the claims contained within it.</li>
                      <li><strong>Cross-domain</strong>: JWTs are easily used across different domains, making them suitable for single sign-on implementations.</li>
                    </ul>
                    
                    <h3>Common Use Cases</h3>
                    <ul>
                      <li><strong>Authentication</strong>: The most common scenario for using JWT. Once a user is logged in, each subsequent request will include the JWT, allowing the user to access routes, services, and resources that are permitted with that token.</li>
                      <li><strong>Information Exchange</strong>: JWTs are a good way of securely transmitting information between parties because they can be signed, ensuring the content hasn't been tampered with.</li>
                    </ul>
                  </div>
                )}
              </div>
